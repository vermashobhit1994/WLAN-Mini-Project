""" ################################## MODULE DESCRIPTION #########################"""
#Module Description : Provides the functionalities related to display window
...
from tkinter import ttk,messagebox,Label,Frame,Button,IntVar,Tk
...
def event_handler_func(event,tab_obj_list,ip_addr,display_frame_tab_ctrl,close_window_btn):    
    ...
    ### NOTE: relheight_treeview,relwidth_treeview,relx_treeview,rely_treeview is order of argments for function reset_data_treeview
    if tab_text == "Show Signal Strength ":
        ...
        if tab_text == "Show Signal Strength ":
            ...
            #Configuration parameters for treeview for signal strength tab
            relheight_treeview = 0.8
            relwidth_treeview = 0.9
            relx_treeview = 0.04
            rely_treeview = 0.05
            ...
            reset_data_treeview(tab_obj_list[0],field_names,relheight_treeview,relwidth_treeview,relx_treeview,rely_treeview)
            ...
            show_signal_strength(tab_obj_list[0],field_names,relheight_treeview,relwidth_treeview,relx_treeview,rely_treeview)
        ...
        if tab_text == " Show Connected Devices ":
            ...
            #Configuration parameters for treeview for show connected devices tab
            relheight_treeview = 0.8
            relwidth_treeview = 0.9
            relx_treeview = 0.04
            rely_treeview = 0.05
...
##############################################################################################################################
#########################Create Display_Window Class start ###################################################

class Display_Window():

    """############################ Display_Window class Init Start here #########################################"""
    def __init__(self,router_ipaddr,tk_obj):
        self.router_ipaddr = router_ipaddr
        self.tk_obj = tk_obj

    
        
        self.tk_obj.configure(background='#5757ce')#background color
    
        full_width  = self.tk_obj.winfo_screenwidth()
        full_height = self.tk_obj.winfo_screenheight()
    
        print(full_width)

        window_title = "DISPLAY DATA WINDOW"
        self.tk_obj.geometry("%dx%d+0+0" % (full_width,full_height) )
        self.tk_obj.title(window_title)

    
        #Create a label for display window title
        top_title = Label(self.tk_obj,text=window_title,font=('helvetica',40,'bold'),bg='#154bcd',fg='white',relief='groove')
        top_title.place(x=0,y=0,relwidth=1)

        #make a frame so that buttons and other widgets can be put in it
        Display_Frame = Frame(self.tk_obj,width=full_width,height=full_height-120,bg='#154bcd')
        Display_Frame.place(y=60)
        ...
        style = ttk.Style()
        width_tab = full_width/5 
        print(width_tab)
        style.configure("TNotebook.Tab",font=('Times New Roman', 27 ,'bold'), foreground = '#0c0c0c',background='#afedfc',width = width_tab )
    
        #creating the control for all the tabs that is being created
        display_frame_tab_ctrl = ttk.Notebook(Display_Frame,width=full_width,height=full_height-120)
        display_frame_tab_ctrl.place(relx=1,rely=1)
    
        
        
        ############ creating the signal strength tab start here #########################
        ...
        ############ creating the signal strength tab ends here ###############################
     
        
        ############ creating the connected devices tab start here #########################
        ...
        display_frame_tab_ctrl.pack()
        ###################### create of connected devices tab ends here ###################
    
        ###################### create of speed test tab start here ###################
        ...
        ###################### create of speed test tab ends here ###################
    

        ###################### create of channel signal strength tab start here ###################
        ...
        display_frame_tab_ctrl.pack()
        ###################### create of channel signal strength tab ends here ###################
    
    
        ###################### create of channel devices tab start here ###################
        ...
        display_frame_tab_ctrl.pack()
        ###################### create of channel devices tab ends here ###################
    
        #####################################################################################################
        ########################### close button code start ##############################################################################

        #close the window upon click on button
        close_window_btn = Button(Display_Frame,text = 'Close',command=close_window,bg = '#162bdc',font = ('Times New Roman', 20 ,'bold'),\
        width=20,fg='black',highlightcolor = 'white')     
        close_window_btn.place(relx=0.4,rely=0.87)
        close_window_btn['command'] = lambda :close_window(self.tk_obj)
        ...
        display_frame_tab_ctrl.bind("<<NotebookTabChanged>>",lambda event,arg1=tab_list,arg2=router_ipaddr,arg3 = display_frame_tab_ctrl,\
        arg4=close_window_btn:event_handler_func(event,arg1,arg2,arg3,arg4))
        ...
        ############################ Create Display Window ends here ###################################################
        #################################################################################################################################
        """############################ Display_Window class Init Ends here #########################################"""
...
def show_speed_test(*args):
    ...
    speed_test_data_list = task4.get_speed_data()
    print(speed_test_data_list)
    ...
    if speed_test_data_list != None:
        try:
            ...
            messagebox.showerror("Speed Checking Error",err)
...
def show_channel_signal_strength(*args):
    ...
    #proceed only if connected to wifi 
    elif signal_strength_both_freq_data != None:
...
relheight_treeview,relwidth_treeview,relx_treeview,rely_treeview):
    ...
    verscrlbar.place(relx=REL_X_TREEVIEW,rely= REL_Y_TREEVIEW ,relheight=REL_HEIGHT_COLUMN_TREEVIEW) 
...
""" ############################## Display_Window class Instance Methods Ends here ###################### """
